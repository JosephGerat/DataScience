...
def negative_log_gradient(xs: List[Vector],
    ...
    os = []
    for x,y in zip(xs, ys):
        o = _negative_log_gradient(x, y, beta)
        #print(f'\t log gradient {o}, beta {beta}')
        os.append(o)
    return vector_sum(os)

from gradient_descent import gradient_step

xs = [[1.0, 0.84, 0.0], [1.0, 0.84, 0.21], [1.0, 0.84, 0.1]]
ys = [1, 0, 1]
beta = [0, 0, 0.0]

for i in range(1000):
    o = negative_log_gradient(xs, ys, beta)
    beta = gradient_step(beta, o, -1.0)
    loss = negative_log_likelihood(xs, ys, beta)
    print(f'[{i}]grad[{o}], beta[{beta}], loss {loss}')
